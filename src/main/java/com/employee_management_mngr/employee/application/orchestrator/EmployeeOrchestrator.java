...
public class EmployeeOrchestrator implements EmployeeUseCase {
    ...
    public void updateEmployeeStatus(Integer employeeId, EmployeeStatus newStatus) {
        try {
            ...
            logger.error("Error updating status to {} for employee ID {}: {}", newStatus, employeeId, e.getMessage(),
                    e);
    ...
    public List<Employee> findEmployeesByAssignedBy(Integer assignedById) {
        try {
            ...
            logger.error("Error finding employees assigned by ID {}: {}", assignedById, e.getMessage(), e);
            throw new ErrorCreationEmployee(
